...
*** =Euclid= for PDC coefficient matrix
    ...
    #+BEGIN_SRC matlab :session *MATLAB* :eval yes :tangle ./tg_s06_Error_evaluation.m :results silent
      ...
      for nn = 1:length(tmp_allFields)
          tmp_error = zeros(sum(SETUP.SRCS(:,1)),sum(SETUP.SRCS(:,1)));
          for ii = 1:sum(SETUP.SRCS(:,1))
              for jj = 1:sum(SETUP.SRCS(:,1))
                  tmp_error(ii,jj) = norm(rec_funDep_PDC.(tmp_allFields{1})(ii,jj,:) - rec_funDep_PDC.(tmp_allFields{nn})(ii,jj,:),'fro')^2;
              end
          end
          rec_funDep_PDC_ErrEuclid.(tmp_allFields{nn}) = mean(mean(tmp_error));
      end
...
*** =CorrCf= for PDC coefficient matrix
    ...
    #+BEGIN_SRC matlab :session *MATLAB* :eval yes :tangle ./tg_s06_Error_evaluation.m :results silent
      ...
      for nn = 1:length(tmp_allFields)
          ...
          for ii = 1:sum(SETUP.SRCS(:,1))
              for jj = 1:sum(SETUP.SRCS(:,1))
                  tmp_x=corrcoef(rec_funDep_PDC.(tmp_allFields{1})(ii,jj,:),rec_funDep_PDC.(tmp_allFields{nn})(ii,jj,:));
                  tmp_error(ii,jj)=tmp_x(1,end);
              end
          end
          rec_funDep_PDC_ErrCorrCf.(tmp_allFields{nn}) = mean(mean(tmp_error));
      end
